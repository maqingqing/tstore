			...
			function formSubmit(){
				...
				function createsuccess(volumeIData){
		    		...
		    		if (volumeIData["status"] == "Started"){
			     	 	tmp["status"] = "<span class='label label-success' data-lang='running'>"+run+"</span>";
			    	} else{
			      		tmp["status"] = "<span class='label label-danger' data-lang='status_stop'>"+stop+"</span>";
			    	}
					...
					$('#' + tmp["volumeName"] + 'Usage').highcharts().reflow();
				}
		...
		vRestartButton.click(function (){	
		    ...
		    function restartSuccess(){
				...
				// };
	          	...
	          	rowData["status"] = "<span class='label label-success' data-lang='running'></span>";
			...
			$.when(stoped()).done(function(data){
				var status = data["success"];
	      		if (status){
	        		...
	        		var rowData = table.bootstrapTable('getRowByUniqueId', cVolumeName);
	        		rowData["status"] = "<span class='label label-danger' data-lang='status_stop'></span>";
	        		table.bootstrapTable('updateByUniqueId', {id: cVolumeName, row:rowData});
	        		$.loadProperties(web_lang);
	      		} else {
	      			$("#loadingPage").css("display","none");
